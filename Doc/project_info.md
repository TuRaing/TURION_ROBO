...
## Broader Indian-language TTS coverage — researched, not yet acted on (2026-08-31)
Piper (current TTS engine) only covers 7 of India's languages: Hindi, Marathi, Bengali, Telugu, Malayalam, Urdu, Nepali. No Tamil, Kannada, Gujarati, Punjabi, Odia, Assamese, Sanskrit, etc. Not an issue for now (builder uses Marathi/English/occasional Hindi), but worth knowing if TURION ever needs to support more Indian languages. Two candidates found, neither tested yet:

- **Meta MMS-TTS** (`facebook/mms-tts-<lang>`, e.g. `mms-tts-mar`, `mms-tts-hin`) — part of Meta's Massively Multilingual Speech project, 1,107 languages total (exact Indian-language count unconfirmed, but broad). Same VITS architecture family as Piper, so likely fast — unverified.
- **AI4Bharat Indic-TTS** (`github.com/AI4Bharat/Indic-TTS`, distinct project from Indic Parler-TTS above — easy to confuse the names) — covers **13 Indian languages**: Assamese, Bengali, Bodo, Gujarati, Hindi, Kannada, Malayalam, Manipuri, Marathi, Odia, Rajasthani, Tamil, Telugu. Uses FastPitch (acoustic model) + HiFi-GAN (vocoder) — both lightweight, non-autoregressive architectures, same speed class as Piper (not like Parler-TTS's slow generative approach) — genuinely promising, unlike Parler-TTS. Setup is more involved than Piper: needs a forked Coqui-TTS-based library (`gokulkarthik/TTS`) via git + editable install, checkpoints hosted on GitHub Releases (not Hugging Face), and likely needs an older Python again (untested against 3.14 yet) — probably means reusing the isolated-environment approach from the Parler-TTS experiment. **Planned as the next thing to test.**
