...
**STL model actually generated, not just planned (2026-09-04).** Rather than depend on Tinkercad (manual/GUI) or the earlier-abandoned Claude Design canvas (blocked on missing Node.js/Bun — see the pan-tilt turret section's history), built the head shell parametrically in Python: `hardware/sisu_head/build_sisu_head.py` uses `trimesh` + `manifold3d` (installed in an isolated venv at `<scratchpad>/cad_env`, kept out of the main TURION `.venv` the same way the TTS experiments were) to construct an ellipsoid head shell (160×112×90mm, 2.5mm wall), boolean-cut with two eye holes (18mm socket / 10mm through-hole, for the camera + a matching dummy lens), two ear bumps with 4.5mm mic holes, a mouth slot (32×14mm) for the speaker grille, and an open bottom (below z=-55mm) sized to mount onto the pan-tilt bracket. Exported and verified **watertight** (required for slicer/printing), volume ~120.7 cm³ (~150g in PLA, consistent with the ₹300-800 shell estimate). Output: `hardware/sisu_head/sisu_head.stl` (ready to open directly in any slicer, e.g. Cura/PrusaSlicer, for printing) and `sisu_head_preview.png` (4-angle render). The script is parametric — every dimension above is a named constant at the top of the file, so re-running it after a measurement change (e.g. once the actual camera/mic module sizes are known) is a one-line edit + rerun, not a redo from scratch.
