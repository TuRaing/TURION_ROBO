...
- **Set up a separate Python 3.12 environment for future Indic Parler-TTS experimentation**, kept fully outside this repo so it can't affect or be affected by the main project (Python 3.14): portable/embeddable Python at `C:\Python312_portable`, with a venv at `D:\parler_tts_env` (`D:\parler_tts_env\Scripts\python.exe`). Nothing installed in it yet beyond pip — this is just the prerequisite groundwork; actually installing/testing `parler-tts` there is unstarted, deliberately deferred (est. 3-6 hours, uncertain payoff — CPU inference speed for a 0.9B-parameter generative TTS model is unverified on this hardware). The regular Python 3.11/3.12 installer failed here (MSI error 0x80070003 / registry access denied on this machine) — the embeddable-zip + get-pip.py + virtualenv route was used instead.
